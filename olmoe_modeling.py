...
class OlmoeSparseMoeBlock(nn.Module):
    def __init__(self, config):
        ...
        # 当设置了该属性时，会在 forward 时用 custom 权重替换最后 token 的 routing 权重
        self.custom_routing_weights = None

    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        batch_size, sequence_length, hidden_dim = hidden_states.shape
        hidden_states = hidden_states.view(-1, hidden_dim)
        # 计算原始的 router_logits 和 routing_weights
        router_logits = self.gate(hidden_states)
        routing_weights = F.softmax(router_logits, dim=1, dtype=torch.float)
        # 如果设置了自定义 routing 权重，则覆盖每个样本最后一个 token 的 routing 权重
        if self.custom_routing_weights is not None:
            # 这里假设 self.custom_routing_weights 是一个字典，例如：{"expert_0": weight0, "expert_1": weight1, ...}
            custom_weights_list = [
                self.custom_routing_weights.get(f"expert_{i}", 1e-10) for i in range(self.num_experts)
            ]
            custom_weights_tensor = torch.tensor(
                custom_weights_list, dtype=routing_weights.dtype, device=routing_weights.device
            )
            custom_weights_tensor = custom_weights_tensor / custom_weights_tensor.sum()
            # 计算每个样本最后一个 token 在 flat tensor 中的索引
            indices = [i * sequence_length + (sequence_length - 1) for i in range(batch_size)]
            # 替换最后 token 的 routing 权重
            routing_weights[indices] = custom_weights_tensor.unsqueeze(0).expand(batch_size, -1)

            # 检测代码：提取替换后的 routing 权重并与预期比较
            applied_custom = routing_weights[indices].clone().detach()
            expected_custom = custom_weights_tensor.unsqueeze(0).expand(batch_size, -1)
            diff = torch.norm(applied_custom - expected_custom, p=2)
            if diff > 1e-6:
                print(f"WARNING: Custom routing weights applied differ! L2 diff = {diff.item():.6f}")
            else:
                print(f"DEBUG: Custom routing weights applied correctly. L2 diff = {diff.item():.6f}")
        
        # 选择 top-k expert 并归一化（如果需要）
        ...
        # 对选中的 expert 进行 one-hot 编码，便于后续索引
        expert_mask = torch.nn.functional.one_hot(selected_experts, num_classes=self.num_experts).permute(2, 1, 0)
    
        # 遍历所有 expert 并计算每个 expert 的输出
